...
`npm run stage1` runs all three in order. Stage 1 outputs are
git-ignored — they are derived data, regenerated on every CI run.
...
src/
  ...
  data/
    pptx-extracted.json              [stage 1 — gitignored]
    master-transcript.json           [stage 1 — gitignored]
    master-transcript.txt            [stage 1 — gitignored]
...
.github/workflows/render.yml         Stage 1 + render -> out/final.mp4 artifact
```
